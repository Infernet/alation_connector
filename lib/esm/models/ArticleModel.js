...
export class ArticleModel {
  ...
  async getById(id) {
    try {
      const {
        data
      } = await this.apiClient.get(`${ALATION_CATALOG_ROUTE[this.entityType]}/${id}/`);
      return data ?? null;
    } catch (error) {
  ...
  async create(params) {
    try {
      if (!Array.isArray(params)) {
        const {
          data: entity
        } = await this.apiClient.post(ALATION_CATALOG_ROUTE[this.entityType] + '/', params);
        return entity;
      }
      ...
      for (const record of params) {
        const {
          data
        } = await this.apiClient.post(ALATION_CATALOG_ROUTE[this.entityType] + '/', record);
        result.push(data);
      }
