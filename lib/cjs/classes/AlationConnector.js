...
class AlationConnector {
  constructor(user, alationURL, options) {
    ...
    const defaultOptions = {
      tokenStoragePath: _path.default.resolve(process.cwd(), 'keys/alation/'),
      jobInterval: 1000,
    ...
    this.accessTokenPath = _path.default.resolve(this.options.tokenStoragePath, `${this.options.tokenName}_access.token.json`);
    this.refreshTokenPath = _path.default.resolve(this.options.tokenStoragePath, `${this.options.tokenName}_refresh.token.json`);
  ...
  async getAccessToken() {
    try {
      if ((0, _fsExtra.existsSync)(this.accessTokenPath)) {
        return JSON.parse((0, _fsExtra.readFileSync)(this.accessTokenPath, {
          encoding: 'utf-8'
        }));
      }
  ...
  async getRefreshToken() {
    try {
      if ((0, _fsExtra.existsSync)(this.refreshTokenPath)) {
        return JSON.parse((0, _fsExtra.readFileSync)(this.refreshTokenPath, {
          encoding: 'utf-8'
        }));
      } else {
  ...
  saveToken(token) {
    ...
    if ((0, _fsExtra.existsSync)(path)) (0, _fsExtra.unlinkSync)(path);
    (0, _fsExtra.outputFileSync)(path, JSON.stringify(token), {
      encoding: 'utf-8'
    });
  }
