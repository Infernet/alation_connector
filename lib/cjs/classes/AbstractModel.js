...
class AbstractModel {
  ...
  async getPagesData(config) {
    try {
      ...
      const next = response.headers[_constants.ALATION_NEXT_PAGE_HEADER_KEY];
      return {
  ...
  async updateCustomFields(body) {
    try {
      const otype = _constants.OBJECT_TYPE[this.entityType];
      let params;
      ...
      const {
        data
      } = await this.apiClient.post(_constants.CUSTOM_FIELD_VALUE_EDIT_ROUTE, params);
      return data;
    } catch (e) {
